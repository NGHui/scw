...
<body>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<button type="button" class="btn btn-primary"
							...
							<i id="add" class="glyphicon glyphicon-plus"></i> 新增
						</button>
						<br>
						<hr style="clear: both;">
									...
									<c:forEach items="${user_info.list }" var="user" >
										<tr>
											...
											<td>
												...
												<button u_d_id="${user.id}" u_d_name="${user.username}" type="button" class="btn btn-danger btn-xs deleteUser">
	...
	<script type="text/javascript">
		...
		$(".assignBtn").click(function(){
			...
			//去权限列分配页面
			var url = "${ctp}/permission/role/toAssignRolePage?uid="+id;
			location.href=url;
		});

		//删除的方法
		$(".deleteUser").click(function deleteUser() {
			//多表连接不能直接删除已经连接到的表,需要先去掉角色所拥有的权限,在进行删除
			//第一步先获取用户的id
            //console.log($(this).attr("u_d_id"));

			var id= $(this).attr("u_d_id");
			var delname = $(this).attr("u_d_name");
            var Url = "${ctp}/permission/user/delete?id="+id;
			if(confirm("确认删除【"+delname+"】这些员工吗?")){
				location.href = Url;
			}
			return false;
		});
